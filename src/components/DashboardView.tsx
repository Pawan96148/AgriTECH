...
export const DashboardView: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="bg-gradient-to-br from-emerald-900 via-emerald-800 to-green-900 text-white rounded-2xl p-5 sm:p-6 shadow-md relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-emerald-100/90 max-w-xl">
              Deterministic rule engine is active for <strong className="text-lime-300">{farms.length} plots</strong> across <strong className="text-lime-300">{totalAcreage.toFixed(1)} acres</strong>.
            </p>
          </div>

          {/* Weather Snapshot Widget inside banner */}
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-4 min-w-[280px] sm:min-w-[340px] flex flex-col justify-between space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs pt-2 border-t border-white/15">
              <div className="bg-emerald-950/40 rounded p-1.5">
                <span className="text-[10px] text-emerald-300 block">Rain Prob</span>
                <span className={`font-bold ${weather.rainProbability >= 50 ? 'text-amber-300' : 'text-white'}`}>
              ...
              <div className="bg-emerald-950/40 rounded p-1.5">
                <span className="text-[10px] text-emerald-300 block">Wind Speed</span>
              ...
              <div className="flex flex-wrap items-center gap-1">
                {(['rainy', 'sunny', 'heatwave', 'windy'] as const).map((s) => (
                  <button
                    ...
                    className={`px-1.5 py-0.5 rounded text-[10px] font-bold transition capitalize ${
                      activeWeatherSim === s
                        ? 'bg-lime-400 text-emerald-950 shadow-xs'
                        : 'bg-emerald-950/60 hover:bg-emerald-900 text-emerald-200'
                    }`}
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  <div key={crop.id} className="border border-lime-200 bg-lime-50/20 hover:bg-lime-50/40 transition rounded-xl p-3.5 space-y-2.5">
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] font-semibold text-stone-600 mb-1">
                        ...
                        <span className="text-emerald-800 font-mono font-bold">{progressPercent}% ({daysElapsed}/{totalDays}d)</span>
                      </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-gradient-to-br from-lime-50 to-emerald-50 rounded-2xl border border-lime-300/80 p-5 space-y-3">
